...
std::string k_yaml_file = "code\\client\\modules\\factory\\test\\data\\test_health_module_factory_data.yaml";
std::string k_yaml_library_file = "code\\client\\modules\\factory\\test\\data\\test_status_effect_library_data.yaml";

TEST(HealthModuleFactory, Test_HealthModuleFactory) {
    ...
    std::vector<std::string> module_list = Module::HealthModuleFactory::get_Instance()->list_modules();
    ...
    for (YAML::iterator iter = data_set.begin(); iter != end; ++iter) {
        ...
        auto module = Module::HealthModuleFactory::get_Instance()->create_health_module(item_name, item["data"]);
        EXPECT_TRUE(module.get() != nullptr);
        EXPECT_EQ(module->get_module_name(), item_name);
    }
